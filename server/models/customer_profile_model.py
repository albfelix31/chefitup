...
class CustomerProfileModel:
    def __init__(self, userId=None):
        ...
        if userId is not None:
            self.dataCur.execute(
                'SELECT * FROM CustomerProfile WHERE userId = ' + "'" + str(userId) + "'")
    ...
    def updateField(self, field, attribute):
        self.dataCur.execute('UPDATE CustomerProfile Set ' + field + ' = ' + "'" +
                             attribute + "'" + "WHERE profileId = " + "'" + str(self.profileId) + "'")
        self.database.commit()

    def initProfile(self, userId, name, deposit):
        self.dataCur.execute('INSERT INTO CustomerProfile(name,address,payment,balance,subscribe,approve,vip,userId) VALUES (' +
                             "'" + str(name) + "'," + "'' , '', " + "'" + str(deposit) + "'," + " '0', '0','0'," + "'" + str(userId) + "'" + ')')
        self.database.commit()

    def getNotApprove(self):
        self.dataCur.execute(
            'SELECT * FROM CustomerProfile WHERE approve = "0"')
        results = self.dataCur.fetchall()
        return results

    def removeCustomer(self, profileId, userId):
        self.dataCur.execute(
            'DELETE FROM CustomerProfile WHERE profileId = ' + "'" + str(profileId) + "'")
        self.dataCur.execute(
            'DELETE FROM User WHERE userId = ' + "'" + str(userId) + "'")
        self.database.commit()

    def getCustomer(self):
        self.dataCur.execute(
            'SELECT * FROM CustomerProfile WHERE approve = "1"')
        results = self.dataCur.fetchall()
        return results
